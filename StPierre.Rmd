...
Ok...Dr. St-Pierre did not actia;;u generate this data from a simple linear regression model: $y_i = \beta_0 + \beta_1{x_i} + e_i$  but  from the following *random coefficients* model:
...
```
Wonderful!  We get the exact same estimates!!!  They sure seem a lot different from the "truth"!  (intercept 0 slope 1).
...
So let's account for the heterogeneity using the fixed effects model approach first presented by StPierre (2001) on **page 745**.
...
trick is to hold residual variance constant to 1 and specify weights as inverse of sampling variances
...
A mixed model analysis more appropriately accounts for 

## Mixed model (1)
Random effects model using mixed models software
trick to hold residual variance constant to 1
